...
class SearchViewModel {
    ...
    private var trackingParams: String?
    ...
    func searchQuery(with query: String, with newFilter: Bool = false) -> SearchStatus {
        ...
        case .music:
            if hasLoadedMusic {return .noSearch}
                YTMManager.shared.search(query: query, filter: .music) { [weak self] ytModels, token, trackingParams in
        ...
        case .video:
            if hasLoadedMv {return .noSearch}
                YTMManager.shared.search(query: query, filter: .mv) { [weak self] ytModels, token, trackingParams in
                    guard let ytModels = ytModels else {
                        self?.resultMv = []
                        self?.bindingSearching?()
                        return
    ...
    func loadMore() {
        guard let token = (self.filter == .music) ? self.tokenMusic : self.tokenMV,
              let trackingParams = trackingParams else {return}
        ...
        YTMManager.shared.searchContinuation(filter: ytFilter,
                                             token: token,
                                             trackingParams: trackingParams) {
            [weak self] ytModels, token, trackingParams in
