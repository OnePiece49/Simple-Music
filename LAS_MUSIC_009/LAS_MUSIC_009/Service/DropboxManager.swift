...
//	private let client = DropboxClientsManager.authorizedClient
	...
	func downloadFile(fileMetadata: Files.FileMetadata,
		...
		//		guard let client = DropboxClientsManager.authorizedClient, let path = fileMetadata.pathDisplay else {
		//			DispatchQueue.main.async { completion(nil) }
		//			return
		//		}
	...
	func getFileThumbnail(fileMetadata: Files.FileMetadata,
		...
		client.files.getThumbnail(path: path, size: .w128h128, mode: .strict, destination: destination)
