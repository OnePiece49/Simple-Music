...
class PlayerController: BaseController {
    ...
    let relativeYT = "https://www.youtube.com/watch?v="
    ...
    private var firstSetupCommander = true
    ...
    private lazy var activityStackView: UIStackView = {
        let stackView = UIStackView(arrangedSubviews: [listSongButton,
                                                       shareButton,
                                                       addSongButton,
                                                       likeButton])
    ...
    private lazy var containerView: UIView = {
        ...
        NSLayoutConstraint.activate([
            ...
            actionStackview.leftAnchor.constraint(equalTo: view.leftAnchor, constant: 24),
            actionStackview.rightAnchor.constraint(equalTo: view.rightAnchor, constant: -24),
    ...
    private func updatePlayer() {
        self.posterImageView.sd_setImage(with: viewModel.currentURL, placeholderImage: UIImage(named: AssetConstant.ic_thumbnail_default), context: .none)
    ...
    private func loadYoutube() {
        let urlString = self.relativeYT + (self.viewModel.currentVideoId ?? "")
    ...
    private func newPlaylist(playlist: PlaylistModel, currentMusic: MusicModel) {
        self.viewModel.newPlaylist(playlist: playlist, currentMusic: currentMusic)
    ...
    private func setupRemoteComanderView(rate: Int = 1, duration: CMTime? = nil) {
        ...
        thumbnailIV.sd_setImage(with: URL(string: viewModel.currentMusic?.thumbnailURL ?? "")) { image, _, _ , _ in
    ...
    private func playMedia() {
        self.setupRemoteComanderView(rate: 1)
        self.handlePlayButtonTapped()
    }

    private func pauseMedia() {
        self.setupRemoteComanderView(rate: 0)
        self.handlePlayButtonTapped()
    }
    ...
    @objc func handleConvertButtonTapped() {
        let convertVC = ConvertSheetController()
    ...
    @objc func miniSizeButtonTapped() {
        ...
        if smallSize {
            UIView.animate(withDuration: 0.2) {
                self.view.frame.origin.y = self.heightDevice -  MiniPlayerView.heightView - 2 *  tabBarVC.heightTabBar - self.insetTop
                tabBarVC.tabBar.frame.origin.y = self.heightDevice  - 2 * tabBarVC.heightTabBar - self.insetTop
            }
        } else {
            UIView.animate(withDuration: 0.2) {
                self.view.frame.origin.y = self.heightDevice -  MiniPlayerView.heightView - 2 * tabBarVC.heightTabBar
                tabBarVC.tabBar.frame.origin.y = self.heightDevice - 2 * tabBarVC.heightTabBar
            }
        }
    ...
    @objc func didTapListSongButton() {
        let listMusicVC = ListMusicController(playlist: viewModel.getCurrentPlaylist(),
    ...
    @objc func didTapShareButton() {
        ...
        if viewModel.isOffline {
            objectsToShare = [viewModel.currentMusic?.absolutePath as Any]
        } else {
            objectsToShare = [self.relativeYT + (viewModel.currentVideoId ?? "")]
        }
    ...
    @objc func handleWhenLikeButtonTapped(notification: Notification) {
        ...
        viewModel.unfavouriteMusic(music: music)
        self.likeButton.setImage(viewModel.likeImage, for: .normal)
    }
    ...
    @objc func handleLikeButtonTapped() {
        let success = viewModel.likeMusic()
        self.likeButton.setImage(viewModel.likeImage, for: .normal)
    }
    
    @objc func handleAddSongButtonTapped() {
        guard let music = viewModel.currentMusic else {return}
		let addToVC = AddToPlaylistController(type: .playlist, music: music)
...
//MARK: - delegate ConvertSheetControllerDelegate
extension PlayerController: ConvertSheetControllerDelegate {
    func didSelectMediaCell(type: ConvertAudioType) {
        guard let outputString = viewModel.currentURL?.deletingPathExtension().lastPathComponent else {return}
