...
extension BoxController {
    ...
    private func saveToRealm(file: File) {
        let name = file.name ?? ""
        guard let url = URL.importFolder()?.appendingPathComponent(name) else {return}
        guard let thumbnailUrl = URL.thumbnailFolder()?.appendingPathComponent(name).appendingPathExtension("png") else {return}
