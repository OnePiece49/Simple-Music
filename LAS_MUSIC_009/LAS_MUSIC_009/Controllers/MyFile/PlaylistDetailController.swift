...
class PlaylistDetailController: BaseController {
	...
	private func setupNavBar() {
		...
		self.navBar = NavigationCustomView(centerTitle: viewModel.playlist.name,
	...
	private func setupTBView() {
		musicTbv.rowHeight = PlaylistDetailTBCell.cellHeight
...
extension PlaylistDetailController {
	...
	private func bindViewModel() {
		viewModel.onDeleteMusic = { [weak self] in
			...
			self.delegate?.playlistDidChange(self, playlist: self.viewModel.playlist)
		...
		viewModel.onToggleShuffle = { [weak self] in
			guard let self = self else { return }
			let imgName = self.viewModel.didShuffle ? AssetConstant.ic_random_selected : AssetConstant.ic_not_random
			self.shuffleBtn.setImage(UIImage(named: imgName)?.withRenderingMode(.alwaysOriginal), for: .normal)
			self.musicTbv.reloadData()
		}

		viewModel.onToggleReplay = {
			// code ...
		}
	}
}
