...
open class FSPagerView: UIView,UICollectionViewDataSource,UICollectionViewDelegate {
    ...
    @IBInspectable
    open var interitemSpacing: CGFloat = -50 {
        didSet {
            self.collectionViewLayout.forceInvalidate()
        }
    }
    ...
    fileprivate var centermostIndexPath: IndexPath {
        ...
        let sortedIndexPaths = self.collectionView.indexPathsForVisibleItems.sorted { (l, r) -> Bool in
            ...
            return abs(ruler-leftCenter + 100) < abs(ruler-rightCenter)
        }
        let indexPath = sortedIndexPaths.first
        if let indexPath = indexPath {
            print("DEBUG: \(indexPath.row) and \(sortedIndexPaths.count) aaa")
            return indexPath
        }
        return IndexPath(item: 0, section: 0)
    }
...
//        function(self,cell as! FSPagerViewCell,index)
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if !self.isPossiblyRotating && self.numberOfItems > 0 {
            ...
            print("DEBUG: \(currentIndex) before")
            if (currentIndex != self.currentIndex) {
                self.currentIndex = currentIndex
                print("DEBUG: \(currentIndex) after")
            } else {
                self.currentIndex = 0
            }
        }
    ...
    open func scrollToItem(at index: Int, animated: Bool) {
        ...
        var contentOffset = self.collectionViewLayout.contentOffset(for: indexPath)  //DEBUG:
...
//        contentOffset.x = contentOffset.x
        print("DEBUG: \(currentIndex) and \(contentOffset.x)")
    ...
    fileprivate func nearbyIndexPath(for index: Int) -> IndexPath {
        ...
        if abs(currentIndex-index - 100) <= self.numberOfItems/2 {  //DEBUG:
            print("DEBUG: current")
